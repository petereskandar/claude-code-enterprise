...
def build_mdm_config(
    ...
    Returns:
        Dictionary of MDM configuration key-value pairs.  The
        inferenceCredentialHelper value uses a platform-appropriate placeholder
        path — callers that write the final config file must substitute the
        actual expanded path for the current user.
    """
    # Use a forward-slash placeholder so the dict is usable on both platforms;
    # platform-specific generators (reg, claude_desktop_config) override this.
...
def generate_claude_desktop_config(output_dir: Path, mdm_config: dict, profile_name: str) -> Path:
    """Generate Windows claude_desktop_config.json for %LOCALAPPDATA%\\Claude-3p\\.

    Claude Desktop on Windows reads enterprise config from
    ``%LOCALAPPDATA%\\Claude-3p\\claude_desktop_config.json``.  The file must
    contain an ``enterpriseConfig`` key with the inference settings, and the
    ``inferenceCredentialHelper`` value must be an **expanded** absolute path
    (not ``%USERPROFILE%``) because Node.js does not expand environment
    variables when spawning subprocesses.

    This function writes a template file.  The install.bat installer uses a
    PowerShell snippet to copy it into the correct location while substituting
    the actual ``%USERPROFILE%`` value for the current user.

    Returns the path to the generated template file.
    """
    bat_name = f"credential-helper-{profile_name}.bat"
    # "__USERPROFILE__" is a sentinel that install.bat replaces at install time
    # with the actual expanded %USERPROFILE% path (e.g. C:\Users\john).
    # We always use the sentinel regardless of build platform so this template
    # is safe to distribute to developers with different home directories.
    helper_rel = f"claude-code-with-bedrock\\{bat_name}"
    helper_abs = f"__USERPROFILE__\\{helper_rel}"
...
def generate_credential_helper_bat(output_dir: Path, profile_name: str) -> Path:
    """Generate a Windows .bat credential helper that calls credential-process.exe.

    Claude Desktop / Cowork calls inferenceCredentialHelper with no arguments and
    expects {"token": "bedrock-api-key-..."} on stdout.  This .bat wrapper forwards
    the call to credential-process.exe --credential-helper, which has botocore
    bundled and produces the correct token without needing Python installed.

    The file is placed in output_dir (the package directory) so install.bat can
    copy it to %USERPROFILE%\\claude-code-with-bedrock\\.
    ...
    bat_content = (
        "@echo off\r\n"
        f'"%USERPROFILE%\\claude-code-with-bedrock\\credential-process.exe"'
        f" --credential-helper --profile {profile_name}\r\n"
    )
    # Write as raw bytes (ASCII) to ensure no BOM — batch files break with BOM.
...
def generate_reg_file(output_dir: Path, mdm_config: dict, profile_name: str = "ClaudeCode") -> Path:
    """Generate a Windows .reg file for Claude Cowork 3P.

    The inferenceCredentialHelper value is overridden to point to the .bat
    wrapper (credential-helper-<profile>.bat) so no Python installation is
    required on end-user machines.  The .bat in turn calls
    credential-process.exe --credential-helper which has botocore bundled.
    ...
    # Build a working copy so we can override the credential helper path
    config = dict(_mdm_keys(mdm_config))
    # On Windows the helper must be a native .bat / .exe — not a Python script.
    # The backslash is a single \; the escape code in this function will double it
    # to \\ in the .reg file, which .reg import resolves back to a single \.
    config["inferenceCredentialHelper"] = (
        f"%USERPROFILE%\\claude-code-with-bedrock\\credential-helper-{profile_name}.bat"
    )
...
def generate_all(output_dir: Path, mdm_config: dict, console: Console, profile_name: str = "ClaudeCode") -> list[str]:
    """Generate all CoWork 3P MDM configuration files.

    Args:
        output_dir: Directory to write files to.
        mdm_config: MDM configuration dictionary.
        console: Rich console for status output.
        profile_name: AWS profile name used for the Windows credential helper.
    ...
    generate_reg_file(output_dir, mdm_config, profile_name=profile_name)
    generated.append("cowork-3p.reg")
    console.print("[green]✓[/green] Generated cowork-3p.reg (Windows MDM/GPO)")

    generate_claude_desktop_config(output_dir, mdm_config, profile_name=profile_name)
    generated.append("claude_desktop_config.json")
    console.print("[green]✓[/green] Generated claude_desktop_config.json (Windows local config)")

    generate_credential_helper_bat(output_dir, profile_name)
    bat_name = f"credential-helper-{profile_name}.bat"
    generated.append(bat_name)
    console.print(f"[green]✓[/green] Generated {bat_name} (Windows credential helper)")
