...
class Profile:
    ...
    distribution_certificate_arn: str | None = None  # Optional existing ACM certificate ARN (skips cert creation)
    ...
    inference_profiles_models: list[str] = field(
        default_factory=list
    )  # Model keys to create profiles for (empty = all enabled models)
    inference_profiles_default_model: str = (
        "sonnet-4-6"  # Model key whose ARN is written to ~/.claude.json on first login
    )
    inference_profiles_provisioner_arn: str = (
        ""  # ARN of the InferenceProfileProvisionerFunction Lambda; required when inference_profiles_enabled is True
    )
