...
def lambda_handler(event, context):
    ...
    # 1. Build ARN → { email, model, groups } map
...
def _build_arn_to_profile_map() -> dict:
    """
    Return { inferenceProfileArn: { "email": str, "model": str, "groups": list } }
    by scanning the InferenceProfileMapping DynamoDB table (populated by the provisioner Lambda).
    """
    result = {}
    try:
        ...
        for item in items:
            ...
            if arn and email:
                result[arn] = {"email": email, "model": model, "groups": groups}
...
def _build_log_events(arn_to_profile: dict, bedrock_metrics: dict, timestamp: datetime) -> list:
    ...
    Includes user.group dimension when groups are available from DynamoDB.
    """
